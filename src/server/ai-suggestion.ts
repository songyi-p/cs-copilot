...
const PROMPT_CACHE_KEY = "cs-copilot-ai-suggestion-v4";
...
- 고객 답변과 설명에는 영문 상태 코드나 JSON 필드명을 노출하지 말고, "결제 완료", "상품 준비 중", "배송 중", "배송 완료"처럼 자연스러운 한국어 업무 표현을 사용하세요.
...
export const buildAiContext = (
  ...
  const order = req.order ? deriveOrderFacts(req.order, evaluatedAt) : null;
  ...
  return {
    ...
    order: order
      ? {
          ...order,
          orderStatus: orderStatusLabel[order.orderStatus] ?? order.orderStatus,
        }
      : null,
